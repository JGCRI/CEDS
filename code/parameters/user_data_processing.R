...
#          that may need adjustment based on the new user data provided.
#
#          Identify the rows that may need adjusting. If we're dealing with
#          data on not the lowest aggregation level, you need any cells that
#          can map to this cell. Rows in the default activity data that will indeed
#          be changed will be further filtered from this subset of data later on during
#          the normalization process.
#
# Params:
#   all_activity_data: the source data that may need adjusting
#   usrdata: the new data that will replace values in all_activity_data
#   Xyears: the years from the new data
#
# Returns: the rows that may need adjusting
#          generated by matching data to trend
getRowsForAdjustment <- function( all_activity_data, usrdata, Xyears ) {

    data_to_adjust <- dplyr::filter( all_activity_data, iso %in% usrdata$iso,
                                     agg_fuel %in% usrdata$agg_fuel)
    data_to_adjust <- data_to_adjust[ , c( getCEDSAggCols(iso = T), Xyears ) ]
