...
# processUserDefinedData()
    ...
    # Read in the data frame
        ...
        try( mapping_iso <- readData( paste0( "user-defined-energy/", filename, "-mapping"), domain = "EXT_IN", extension = ".xlsx",
                                     sheet_selection = "iso"))
        try( mapping_agg_sector <- readData( paste0( "user-defined-energy/", filename, "-mapping"), domain = "EXT_IN", extension = ".xlsx",
                                            sheet_selection = "agg_sector"))
        try( mapping_ceds_sector <- readData( paste0( "user-defined-energy/", filename, "-mapping"), domain = "EXT_IN", extension = ".xlsx",
                                             sheet_selection = "CEDS_sector"))
        try( mapping_agg_fuel <- readData( paste0( "user-defined-energy/", filename, "-mapping"), domain = "EXT_IN", extension = ".xlsx",
                                          sheet_selection = "agg_fuel"))
        try( mapping_ceds_fuel <- readData( paste0( "user-defined-energy/", filename, "-mapping"), domain = "EXT_IN", extension = ".xlsx",
    ...
    ### TODO: do we write out the results somewhere? keep in memory? Only execute this when we're ready for the data?
    ###       (I like soln #3)
    }
...
mapToCEDS <- function( dataframe, MSL, MCL, MFL, iso_map = NULL, agg_sector_map = NULL, CEDS_sector_map = NULL, 
        ...
        Xyears <- colnames(dataframe)[which(colnames(dataframe) %!in% dataframe_categories)]
      
    # If the iso is not already in the data, map to it
        if ( 'iso' %!in% dataframe_categories ) {
            if ( is.null( iso_map ) ) stop( "Error in mapToCEDS: iso not in data, but no iso map provided" )
