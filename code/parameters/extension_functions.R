# ----------------------------------------------------------------------------------
# CEDS R header file:  extension_functions.R
...
# -----------------------------------------------------------------------------
 ...
 H.process_bond_data <- function( a.ceds_extension_fuels = ceds_extension_fuels,
   ...
   bond_import <- readData( 'EXT_IN', 'CD.Bond_country_industrial_biomass' )
   ...
   # Remove white spaces
   bond_import$Country <- trimws(bond_import$Country, which=c("both"))
   bond_import$Sector <- trimws(bond_import$Sector, which=c("both"))
   bond_import$Tech <- trimws(bond_import$Tech, which=c("both"))
   bond_import$Fuel <- trimws(bond_import$Fuel, which=c("both"))

   bond_sector_map$Sector <- trimws(bond_sector_map$Sector, which=c("both"))
   bond_sector_map$Tech <- trimws(bond_sector_map$Tech, which=c("both"))

   fuel_map$Fuel <- trimws(fuel_map$Fuel, which=c("both"))
   fuel_map$fuel <- trimws(fuel_map$fuel, which=c("both"))
...
#5. Dissaggregate fuel into fuel types, based on combined_fuel_percentages_corrected
  ...
  H.ext_sector_breakdown <- function(a.extension_start_year = extension_start_year,
    ...
    if(a.extension_fuel_category == 'natural_gas'){
      ...
      bond_sector_percentages_full[which(bond_sector_percentages_full$iso == 'usa'), paste0('X', c(1850,1910))]<- c(0,0,1,0,0,0,0)
 ...
 # a.ext_sector_breakdown = ext_sector_breakdown
  ...
  compute_feedstock_split <- function( A.comb_activity, IEA_en_stat_ctry_hist,
                                       IEA_product_fuel, IEA_fuel_list, ceds_extension_fuels, iea_start_year, back_extension_year=1750 ){

    other_sector <- "1A1bc_other_feedstock"
    all_countries <- unique(A.comb_activity$iso)

    # Extract NONENUSE for IEA products
    IEA_en_stat_ctry_fs <- IEA_en_stat_ctry_hist[ IEA_en_stat_ctry_hist$FLOW == "NONENUSE" &
                                                    IEA_en_stat_ctry_hist$PRODUCT %in% IEA_fuel_list, ]

    # Aggregate by iso
    XYears <- names(IEA_en_stat_ctry_fs)[grepl('X', names(IEA_en_stat_ctry_fs) )]

    # Map IEA Product to CEDS fuel
    feedstock_by_iso_fuel_sector <- mutate( IEA_en_stat_ctry_fs,  sector = FLOW ) %>%
      left_join( IEA_product_fuel[c( "fuel","product" )], by = c( "PRODUCT" = "product" )  )

    feedstock_by_iso_fuel_sector <- feedstock_by_iso_fuel_sector[c("iso", "sector", "fuel", XYears)]

    feedstock_by_iso_fuel_sector[, X_BP_years] <- feedstock_by_iso_fuel_sector[, X_IEA_end_year ]

    feedstock_by_iso_fuel_sector$units <- "kt"

    XYears <- c(XYears, X_BP_years )

    # IEA_en_stat_ctry_all  <- mutate( IEA_en_stat_ctry_all,  sector = FLOW ) %>%
    #                                  left_join( IEA_product_fuel[c( "fuel","product" )], by = c( "PRODUCT" = "product" )  ) %>%
    #                                  select( -PRODUCT, -FLOW )

    ceds_iea_total_by_sector <- bind_rows( feedstock_by_iso_fuel_sector, A.comb_activity)

    ceds_iea_total_fuel_agg <- aggregate( ceds_iea_total_by_sector[ XYears ], by = list( iso = ceds_iea_total_by_sector$iso,
                                                                                         fuel = ceds_iea_total_by_sector$fuel ), FUN = sum )

    # Aggregate by iso , fuel, and Flow
    feedstock_sector_agg <- aggregate( feedstock_by_iso_fuel_sector[ XYears ], by = list( iso = feedstock_by_iso_fuel_sector$iso,
                                                                                          fuel = feedstock_by_iso_fuel_sector$fuel), FUN = sum )

    feedstock_sector_split <- left_join( feedstock_sector_agg, ceds_iea_total_fuel_agg, by = c("iso", "fuel") )
    feedstock_sector_split[ XYears ] <- ( feedstock_sector_split[paste0(XYears,".x")] /
                                            feedstock_sector_split[paste0(XYears,".y")] )
    feedstock_sector_split$sector <- other_sector
    feedstock_sector_split <- feedstock_sector_split[ c("iso", "sector", "fuel", XYears )]

    # Replace NaNs and NAs with zero
    feedstock_sector_split  <- replace( feedstock_sector_split, feedstock_sector_split == "NaN", 0);

    # Linearly extend the sector splits backward from 1960/1971
    feedstock_sector_split_extended <- lapply( unique(iea_start_year$start_year), function(iea_year){

      extension_years <- paste0("X", back_extension_year: (iea_year-1))

      country_isos <- iea_start_year[iea_start_year$start_year == iea_year, "iso"]

      feedstock_sector_split_selected <- feedstock_sector_split[ feedstock_sector_split$iso %in% country_isos,  ]

      feedstock_sector_split_selected[ extension_years ] <- feedstock_sector_split_selected[ paste0("X",iea_year ) ]

      return(feedstock_sector_split_selected)

    } )

    feedstock_sector_split_extended <- do.call( rbind, feedstock_sector_split_extended )


    # Create a template to include all CEDS countries with no feedstock data

    feedstock_sector_split_template <- data.frame( iso = rep( all_countries , each = length(ceds_extension_fuels) ),
                                                   sector = rep( x=other_sector, times = length(all_countries) ),
                                                   fuel = rep( x=ceds_extension_fuels, each = length(other_sector)),
                                                   stringsAsFactors = F )

    feedstock_sector_split_final <- full_join(feedstock_sector_split_template, feedstock_sector_split_extended)

    feedstock_sector_split_final[ is.na(feedstock_sector_split_final) ] <- 0

    # Linearly extend the sector splits backward from 1960/1971
    feedstock_sector_split_final <- feedstock_sector_split_final[c("iso", "sector", "fuel", paste0('X',back_extension_year:1971))]

    return(feedstock_sector_split_final)
 }#compute_feedstock_split() Ends