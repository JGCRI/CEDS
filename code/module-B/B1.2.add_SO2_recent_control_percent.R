...
last_inv_year_csv <- readData( 'MAPPINGS', 'SO2_control_frac_last_inv_year', extension = '.xlsx', sheet_selection = 1 )
exclude_sector_fuel_combination <- readData( 'MAPPINGS', 'SO2_control_frac_exclude_fuel_sector' )
...
# exteact recent year gains ef 
gains_recent <- gains_ef_db[ , c( 'iso', 'sector', 'fuel', 'units', recent_Xyears  ) ] 

# extract the last year ef value for each country 
last_year_ef_list <- c( )
for ( row_index in 1 : nrow( gains_recent ) ) {
  iso_name <- gains_recent[ row_index, 'iso' ] 
  last_inv_year <- last_inv_year_csv[ last_inv_year_csv$iso == iso_name, 'last_inv_year' ]  
...
for ( row_index in 1 : nrow( recent_ratio ) ) {
  ...
  years_before_lastinvyear <- as.numeric( recent_years[ 1 ] ) : last_inv_year 
...
recent_ratio <- cbind( gains_recent[ , c( 'iso', 'sector', 'fuel', 'units' ) ],
                       last_inv_year_list, 
                       recent_ratio )
colnames( recent_ratio ) <- c( 'iso', 'sector', 'fuel', 'units', 'last_inv_year', recent_Xyears )
...
control_db_merge <- merge( control_db[ , c( 'iso', 'sector', 'fuel', recent_Xyears ) ], recent_ratio, by = c( 'iso', 'sector', 'fuel' ) )
...
for ( row_index in 1 : nrow( control_db_merge ) ) {
  ...
  last_year_cf <- control_db_merge[ row_index, paste0( 'X', last_inv_year, '.x' ) ]
  last_year_cf_list <- c( last_year_cf_list, last_year_cf )
  }
# make into matrix form 
control_db_last_year_cf <- matrix( rep( last_year_cf_list, length( recent_years ) ), ncol = length( recent_years ) )

# generate a all 1 matrix 
all_one_mat <- matrix( 1, dim( control_db_last_year_cf )[ 1 ], dim( control_db_last_year_cf )[ 2 ] )

# calculate recent controls
recent_control <- all_one_mat - ratio_mat + ratio_mat * control_db_last_year_cf 
# apply the calc_mask to mask out the new controls that should not be calculated 
recent_control <- recent_control * calc_mask 

recent_controls <- cbind( control_db_merge[ , c( 'iso', 'sector', 'fuel' ) ], recent_control )
...
merge_table <- merge( control_db, recent_controls, by = c( 'iso', 'sector', 'fuel' ), all = T )
...
merge_no_na[ , paste0( 'X', recent_years, '.x') ] <- 
  merge_no_na[ , paste0( 'X', recent_years, '.x') ] * original_mask +
  recent_mat * recent_mask
...
final_control <- final_control[ , c( 'iso', 'sector', 'fuel', 'units', db_years ) ]
