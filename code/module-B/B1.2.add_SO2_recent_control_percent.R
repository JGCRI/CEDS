...
# Remove undesired sector fuel combination from gains_ef_db
    for ( row_index in 1 : nrow( exclude_sector_fuel_combination ) ) { #TODO: (previous note:) This does not need to be a for loop!
        gains_ef_db <-
...
# Remove NAs generated by unexisiting iso-sector-fuel combination in control_db ( if any )
    merge_no_na <- merge_table[ !is.na( merge_table[ , paste0( "X", start_year ) ] ), ]
