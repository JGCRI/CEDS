...
# Date Last Updated: November 22, 2019
...
#   E.) Define function to replace column names for year columns
...
#       other missing fuels to zero
        ...
        missing_road_set_zero_1 <- rbind(
            missing_road_set_diesel_oil %>%
                dplyr::mutate( fuel = "biomass" ) %>%
                dplyr::mutate( EF = 0 ),
            missing_road_set_diesel_oil %>%
                dplyr::mutate( fuel = "coal_coke" ) %>%
                dplyr::mutate( EF = 0 ) )

        missing_road_set_zero_2 <- rbind(
            missing_road_set_zero_1,
            missing_road_set_zero_1 %>%
                dplyr::filter( fuel == "biomass" ) %>% dplyr::mutate( fuel = "brown_coal" ) %>%
                rbind( missing_road_set_zero_1 %>% dplyr::filter( fuel == "biomass" ) %>%
...
#     I. Initial cleaning / reformatting of off-road fuel consumption data
      ...
      #TODO: Remove this position indexing, and replace with value indexing
...
#   A.) Apply the EFs to all CEDS years and remove years not needed (currently 1960-2014)
    ...
    oldest_current_year <- min( current_years )

    comb_ef_wide <- comb_ef %>%
    ...
    if( most_recent_current_year < end_year ){
