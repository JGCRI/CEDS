...
# cdiac
cdiac_combustion <- cdiac[ which( cdiac$fuel %in% c("solid_fuels") ) , ]
...
for (i in seq_along(year_intervals)) {
  ...
  drivers <- extension_drivers[ which( paste(extension_drivers$ext_start_year,extension_drivers$ext_end_year,sep='-') == interval ), ]
  ...
  extention_data <- activity[ which( paste(activity$sector, activity$fuel, sep="-") 
                                                       %in% sectors  ) , ]
