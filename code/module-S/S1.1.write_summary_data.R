...
if ( length( list.files( "../final-emissions/current-versions/", pattern = paste0( "_", em, "_" ) ) ) == 0 ) {
  ...
  # read last-run emissions summary and gather descriptor info for comparison
  em_current <- Em_by_Country_Sector
  ...
  if ( any( sort( id_cols ) != sort( id_cols_last ) ) ) {
    ...
    # else write the new summary and run comparison diagnostics
  } else {
