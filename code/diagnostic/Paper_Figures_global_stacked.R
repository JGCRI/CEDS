# ------------------------------------------------------------------------------
# Program Name: Paper_Figures_global.R
...
sector_ceds_long$em <- em
sector_ceds_long$Inventory <- NA
...
if( em == 'CO2'){
  Inventory_plot <- ggplot(data= df, aes(x=year,y=total_emissions, linetype = Inventory, shape = Inventory)) + 
    ...
    scale_fill_discrete(name = 'Sector',na.value = 'white')+
  ...
  Sector_plot <- ggplot(data= df, aes(x=year,y=total_emissions)) + 
    geom_area( data= df2, aes(x=year,y=total_emissions, fill = sector),  alpha = .7)+
    scale_fill_discrete(name = 'Sector')
  ...
  plot <- ggplot(data= df, aes(x=year,y=total_emissions, linetype = Inventory, shape = Inventory)) + 
    ...
    # scale_linetype_manual("Inventory", values = c(1,0))
    # 
    # scale_shape_manual("Inventory",values=c(CDIAC = NA ,CMIP5= 19))
    # plot + guides(color=guide_legend(override.aes=list(shape=c(NA,16),linetype=c(1,0))))
    # 
    # values=c(CDIAC = 'solid' , CMIP5 = 'blank')
...
grid.arrange(total_comparison_nolegend_list[[1]],total_comparison_nolegend_list[[2]],total_comparison_nolegend_list[[3]],
             ...
             total_comparison_nolegend_list[[7]],total_comparison_nolegend_list[[8]],leg,
             ncol=3)
dev.off()
...
grid.arrange(total_stacked_sector_nolegend_list[[1]],total_stacked_sector_nolegend_list[[2]],total_stacked_sector_nolegend_list[[3]],
             ...
             grid.arrange(Sector_plot_lend,Inventory_plot_legend, ncol=1),
             ncol=3)
dev.off()

# Stacked Region
leg <-g_legend(total_stacked_region_list[[1]])
# Stacked Region 
# 3x3
leg <-g_legend(total_stacked_region_list[[1]])
pdf(paste0('../diagnostic-output/paper-figures/Paper_Figures_RCP_Comparison_stacked_region 3x3_legend.pdf'),width=9.5,height=9.5,paper='special')
grid.arrange(total_stacked_region_nolegend_list[[1]],total_stacked_region_nolegend_list[[2]],total_stacked_region_nolegend_list[[3]],
             ...
             total_stacked_region_nolegend_list[[7]],total_stacked_region_nolegend_list[[8]],leg,
             ncol=3)
dev.off()

# Stacked fuel
leg <-g_legend(total_stacked_fuel_list[[1]])
# Stacked fuel 
# 3x3
leg <-g_legend(total_stacked_fuel_list[[1]])
pdf(paste0('../diagnostic-output/paper-figures/Paper_Figures_RCP_Comparison_stacked_fuel 3x3_legend.pdf'),width=9.5,height=9.5,paper='special')
grid.arrange(total_stacked_fuel_nolegend_list[[1]],total_stacked_fuel_nolegend_list[[2]],total_stacked_fuel_nolegend_list[[3]],
             ...
             total_stacked_fuel_nolegend_list[[7]],total_stacked_fuel_nolegend_list[[8]],leg,
             ncol=3)
dev.off()
