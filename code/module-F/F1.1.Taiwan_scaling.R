...
# the Taiwan region from latest emissions working copy by using Taiwan emission inventory data.
# This data only contains data from 2003,2006,2010.
# Input Files: emissions_scaling_functions.R, F.[em]_scaled_EF.csv, 
#              F.[em]_scaled_emissions.csv, Taiwan_scaling_mapping.xlsx, 
...
log_msg <- "Taiwan inventory scaling" # First message to be printed to the log
script_name <- paste0(em,"-F1.1.Taiwan_scaling.R")
...
if ( em %!in% c( 'SO2', 'NOx', 'NMVOC', 'CO' ) ) {
  stop (paste( ' Taiwan scaling is not supported for emission species', em, 'remove from script
...
sector_fuel_mapping <- 'Taiwan_scaling_mapping'
