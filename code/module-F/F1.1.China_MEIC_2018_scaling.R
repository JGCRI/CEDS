#------------------------------------------------------------------------------
# Program Name: F1.1.China_MEIC_2018_scaling.R
# Authors' Names: Erin McDuffie (adapted from F1.1.China_scaling.R)
# Date Last Modified: July 15, 2020
# Program Purpose: To create scaling factors and update emissions estimate for
#                  the China region from latest emissions working copy by using aggregate
# Input Files: emissions_scaling_functions.R, F.[em]_scaled_EF.csv,
#              F.[em]_scaled_emissions.csv, MEIC_2018_scaling_mapping.csv,
#              MEIC_2018_NOx_scaling_mapping.csv, E.[em]_CHN_2018_inventory.csv
# Output Files: F.[em]_total_scaled_EF.csv, F.[em]_total_scaled_emissions.csv
# Notes:
#   (1) MEIC trends inventory data.
#   (2) This data only contains data from 2010-2017.
#   (3) Units are initially in Mg or Metric Tonnes
# ------------------------------------------------------------------------------
...
# provide logging, file support, and system functions - and start the script log.
    ...
    log_msg <- "China inventory scaling - recent years" # First message to be printed to the log
    script_name <- paste0( em, "-F1.1.China_MEIC_2018_scaling.R" )
...
# Stop script if running for unsupported species
    if ( em %!in% c( 'SO2', 'NOx', 'NH3', 'NMVOC', 'CO', 'BC', 'OC' ) ) {
        stop( paste( 'CHN scaling is not supported for emission species ',
...
#   the inventory (as a vector of iso codes)
    inv_name <- 'CHN_2018' #for naming diagnostic files
    region <- c( "chn" )
    inventory_data_file <- paste0( 'E.', em, '_', inv_name, '_inventory' )
    inv_data_folder <- 'MED_OUT'
    sector_fuel_mapping <- 'MEIC_2018'
    mapping_method <- 'sector'
    inv_years <- c( 2010 : 2017 )

    # Species-sepcific mapping files
    if ( em %in% c( 'SO2', 'NMVOC' ) ) {
