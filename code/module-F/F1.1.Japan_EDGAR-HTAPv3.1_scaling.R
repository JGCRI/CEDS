#------------------------------------------------------------------------------
# Program Name: F1.1.Japan_EDGAR-HTAPv3_scaling.R
...
#                  the Japan from latest emissions working copy by using
#                  aggregate inventory data.
# Input Files: emissions_scaling_functions.R, F.[em]_scaled_EF.csv,
#              F.[em]_scaled_emissions.csv, S_Japan_scaling_mapping.csv,
...
log_msg <- "Japan HTAP inventory scaling..." # First message to be printed to the log
script_name <- paste0( em, "-F1.1.Japan_EDGAR-HTAPv3.1_scaling.R" )
...
sector_fuel_mapping <- 'EDGAR_HTAPv3.1_jpn'
inv_name <- 'EDGAR_HTAPv3.1'
region <- c( "jpn")
inv_years <- c( 2000 : 2020 )

inventory_data_file <- paste0( 'E.', em, '_', inv_name, '_inventory' )
inv_data_folder <- 'MED_OUT'
inv_name <- 'EDGAR_HTAPv3.1_jpn' #for naming diagnostic files
