...
# Date Last Modified: Aoril 30, 2020
...
#                  aggregate CAN trends inventory data. This file uses the older
#                  Canadian inventory data used until 2011. This data extends
#                  back to 1985.
# Input Files: emissions_scaling_functions.R, F.[em]_scaled_EF.csv,
#              F.[em]_scaled_emissions.csv, CAN_scaling_mapping.csv,
#              E.[em]_CAN_to2011_inventory.csv
...
# provide logging, file support, and system functions - and start the script log.
    ...
    log_msg <- "Canada inventory scaling (older data)..." # First message to be printed to the log
    script_name <- paste0( em, "-F1.1.CAN_scaling_olderData.R" )
...
#   the inventory (as a vector of iso codes)
    sector_fuel_mapping <- 'CAN_old'
    mapping_method <- 'sector'
    inv_name <- 'CAN_old_to2011' #for naming diagnostic files
    region <- c( "can" )
