# Header --------------------------------------------------------------------
# Program Name: G4.2.downscale_subVOC_emissions.R
...
if ( is.na( VOC_em ) ) VOC_em <- "VOC04"
initialize( paste0("G4.2.downscale_", VOC_em, "_emissions.R"), log_msg, headers )

# Load raster library (for some reason not read in by default through renv)
library('raster')

# Set emission species to NMVOC
em <- "NMVOC"
...
# 3. Initialize NetCDF file function ---------------------------------------
...
for(year in final_years){
    ...
    for(sector in sector_list) {
        ...
        file_name <- paste( em, VOC_em, sector, year, '0.5', sep = '_' )
        ...
        if (sector %in% "ENE") {
            ...
            remove(ELEC_proxy, ETRN_proxy, FFFI_proxy)
        ...
        # Remove temp grids from memory
        suppressWarnings( rm(downscale1, downscale2, downscale3,
                             input_grid,
                             primary_proxy, primary_backup, default_proxy) )
        ...
        # Remove flux grids from  memory
        rm(flux, flux_with_sources, flux_with_seasonality)
    ...
    summary_name <- paste0( output_dir, 'CEDS_', VOC_em, '_anthro_', year, '_', new_res, '.csv' )
    write.csv( total_month_em, file = summary_name, row.names = F )

    # 3.3 Create output NetCDF files -----------------------------------

    # Generate nc file name
    nc_file_name <- paste0( 'CEDS_', VOC_em, '_anthro_', year, '_', new_res, '.nc' )
    ...
    # species information
    reporting_info <- data.frame( em_species = c( 'SO2', 'NOx', 'CO', 'NMVOC', 'NH3', 'BC', 'OC', 'CO2', 'CH4', 'N2O' ),
                                  info = c( 'Mass flux of SOx, reported as SO2',
                                            'Mass flux of NOx, reported as NO2',
                                            'Mass flux of CO',
                                            'Mass flux of NMVOC (total mass emitted)',
                                            'Mass flux of NH3',
                                            'Mass flux of BC, reported as carbon mass',
                                            'Mass flux of OC, reported as carbon mass',
                                            'Mass flux of CO2',
                                            'Mass flux of CH4',
                                            'Mass flux of N2O' ),
                                  stringsAsFactors = F )
...
checksum_df <- list.files( output_dir, paste0( '_', VOC_em, '_anthro_.*_0.1.csv' ), full.names = TRUE ) %>%
    lapply( read.csv ) %>%
    dplyr::bind_rows() %>%
