...
scalePopNA <- function( df ) {  # df in long format
  ...
  if ( nrow( df ) >= 2 ){
    ...
    for ( i in seq( 2, nrow( df ) ) ) {
      if ( is.na( df$value[[ i ]] ) )
        df$value[[ i ]] <- df$value[[ i-1 ]] * df$pop[[ i ]] / df$pop[[ i-1 ]]
    }
    df <- arrange( df, year )
    for ( i in seq( 2, nrow( df ) ) ) {
      if ( is.na( df$value[[ i ]] ) )
...
smoothRunningAvg <- function( df, k, val_var, group_vars, align = "left" ) {
  df <- ddply( df, group_vars, function( df ) {
    ...
    df_zoo <- rollapply( df_zoo, width = k, FUN = mean, fill = NA, align = align, partial = T, na.rm = T )
...
simplePlot <- function( df, value_var, group_var, title, fn = NA ) {
  p <- ggplot( df, aes_string( x = "year", y = value_var, group = group_var ) ) + 
    ...
    xlab( "year" ) + ylab( paste0( "pulp/paper consumption (", df$units[[1]], ")" ) ) +
    ggtitle( title )
  if ( !is.na( fn ) )
...
# 1. Input
  library( "zoo" )
  # prod_hist_input <- readData( "ACTIVITY_IN", "Wood_Pulp_Consumption", ".xlsx" )[ 2:142, c( 1, 4:161 ) ]
...
# Clean up population
  UN_pop <- select( UN_pop, iso, year, pop ) %>% filter( year <= max( emissions_years ) )
...
# Add all years to FAO data
  ...
  fao_full <- data.frame( year = pulp_years ) %>%
    merge( data.frame( flow = c( "production", "imports", "exports" ) ) ) %>%
...
# Diagnostics: Write out original data and data that get dropped
  diag_FAO_all_original <- cast( fao_full, flow + iso ~ year ) %>% arrange( iso, flow )
  diag_FAO_dropped <- filter( diag_FAO_all_original, iso %!in% Master_Country_List$iso )
  
# Drop countries not in MCL
  fao_full <- filter( fao_full, iso %in% Master_Country_List$iso )
  
# For now, add up composite countries
...
# For imports/exports, scale NA with population then do 4-year running average
  imp <- filter( fao_full, flow == "imports" ) %>%
    ...
    smoothRunningAvg( k = 4, val_var = "value", group_vars = c( "iso", "flow" ) )
...
# Combine production, import, exports into one df
  ...
  fao_full_fixed$country <- Master_Country_List$Country_Name[ match( fao_full_fixed$iso, Master_Country_List$iso ) ]
  fao_full_fixed$region <- Master_Country_List$Region[ match( fao_full_fixed$iso, Master_Country_List$iso ) ]
  
# Diagnostics: Write out smoothed FAO production, import, exports
  diag_FAO_all_fixed <- cast( fao_full_fixed, flow+iso+units~year, value = "value" )
...
# Break out time series or major countries (in iso_driver_list) and regional
  ...
  consumption_region <- filter( consumption, iso %!in% iso_driver_list ) %>% 
    group_by( region, units, year ) %>% summarise( cons = sum( cons ), pop = sum( pop ) ) %>%
    mutate( cons_pc = cons / pop ) %>% filter( !is.na( cons_pc ) )
  
# Smooth per-capita consumption by local regression (loess) with coefficient 1/4
  consumption_ctry_loess <- ddply( consumption_ctry, .(iso, country, region, units), function( df ){ 
    within( df, { cons_pc_smooth <- smoothLoessSingle( df, "cons_pc", coeff = 1/4 ) } )
  })
  consumption_region_loess <- ddply( consumption_region, .(region, units), function( df ){ 
    within( df, { cons_pc_smooth <- smoothLoessSingle( df, "cons_pc", coeff = 1/4 ) } )
  })
...
# Where cons_pc_smooth is <= 0, extend nearest available year constant
  ...
  consumption_ctry_loess_fixed$cons_pc_smooth[ consumption_ctry_loess_fixed$cons_pc_smooth <= 0 ] <- NA
  consumption_ctry_loess_fixed <- ddply( consumption_ctry_loess_fixed, .(iso, country, region, units), function( df ){
  ...
  consumption_region_loess_fixed$cons_pc_smooth[ consumption_region_loess_fixed$cons_pc_smooth <= 0 ] <- NA
  consumption_region_loess_fixed <- ddply( consumption_region_loess_fixed, .(region, units), function( df ){
    df$cons_pc_smooth <- na.locf( df$cons_pc_smooth, na.rm = F )
    df$cons_pc_smooth <- na.locf( df$cons_pc_smooth, na.rm = F, fromLast = T )
    return( df )    
  } )
...
# Compute smoothed total consumption
  consumption_region_loess_tot <- select( consumption_region_loess_fixed, region, units, year, cons_pc_smooth ) %>%
  ...
  consumption_ctry_loess_tot <- mutate( consumption_ctry_loess_fixed, cons = cons_pc_smooth*pop )
  
# Combine country and regional df
  consumption_smooth_tot <- bind_rows( consumption_ctry_loess_tot, consumption_region_loess_tot ) %>%
...
# Cast to wide
  replace <- cast( consumption_smooth_tot, iso+activity+units ~ year, value = "cons" )
...
# Make df of population ratios  
  ...
  pop_ratio[, X_ext_years ] <- pop_ratio[, X_ext_years ] / pop_ratio[, X_ratio_year ]
