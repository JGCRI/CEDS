...
# Define values
    ...
    IEA_coal_list <- c( "Brown coal (if no detail) (kt)", "Coking coal (kt)",
                        ...
                        "Sub-bituminous coal (kt)", "Lignite (kt)", "Anthracite (kt)") # Patent fuel is not included as it is a secondary fuel
    ...
    # TODO: need to pay attention to primary and secondary fuels - refinery gas is a secondary fuel
...
#                   - primary oil NONENUSE - secondary oil NONENUSE
    ...
    CEDS_oil <- dplyr::filter( en_biomass_fsu_fix, fuel %in% CEDS_oil_list ) %>%
