...
# Output Files:  A.comb_activity_extended_oil.csv
...
log_msg <- "Extending oil data with CDIAC and UNSD data..." # First message to be printed to the log
...
#   TODO: (Future) This data isn't used really, as it all sums to 0 when making the list of UN countries.
#         Is this accurate? Should it all sum to 0? Or was there supposed to be non-zero activity data?
...
# Check to make sure that light_oil, diesel_oil, and heavy_oil other_transformation consumption sums
# to zero and that the aggregate fuel "oil" does not have consumption data for any other sector other than other_transformation.
# In other fuels, we were able to seperate other tranformation into CEDS fuel types (i.e. hard coal and brown coal).
other_transformation_check1 <- CEDS_default_actvity %>%
    dplyr::filter( fuel != 'oil' & sector == '1A1bc_Other-transformation' ) %>%
...
if( other_transformation_check1 != 0 ){

    stop( 'In oil extension, light, diesel, or heavy oil is disaggregated to Other-tranformation. Should only be "oil". Please check...' )
...
if( other_transformation_check2 != 0 ){

    stop( 'In oil extension the aggregate fuel "oil" has non-zero consumption data for sectors other than ',
          '1A1bc_Other-transformation. This should not occur, please check...' )
...
CEDS_default_actvity_final <- CEDS_default_actvity %>%
    ...
    dplyr::filter( !( sector == "1A1bc_Other-transformation" & fuel != "oil" ),
