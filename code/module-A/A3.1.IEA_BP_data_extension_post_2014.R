#------------------------------------------------------------------------------
# Program Name: A3.1.IEA_BP_data_extension_post_2014.R
...
# provide logging, file support, and system functions - and start the script log.
    ...
    script_name <- "A3.1.IEA_BP_data_extension_post_2014.R"
...
# 1. Read in files
    ...
    iea_data_full <- readData( "MED_OUT", "A.IEA_BP_energy_ext_2014" )
    ...
    bp_oil_full  <- readData( "ENERGY_IN", bp_energy_data_file_name, ".xlsx", sheet_selection = 7 ) # Oil Consumption- Tonnes
    printLog( c("Read in BP data sheet: ", names( bp_oil_full )[[1]]) )
    bp_gas_full  <- readData( "ENERGY_IN", bp_energy_data_file_name, ".xlsx", sheet_selection = 24 ) # Gas Consumption – tonnes
    printLog( c("Read in BP data sheet: ", names( bp_gas_full )[[1]]) )
    bp_coal_full <- readData( "ENERGY_IN", bp_energy_data_file_name, ".xlsx", sheet_selection = 33 ) # Coal Consumption -  Mtoe
...
# Define BP years
  BP_years <- BP_years[BP_years != 2014 ]
  X_BP_years <- paste0('X',BP_years)
  IEA_end_year <- 2014  
  X_IEA_end_year <- 'X2014'
  IEA_years <- c(IEA_years,2014)
...
# Reduce IEA data to only what's needed to calculate projections
    iea_data <- iea_data_full[ iea_data_full[, 'iso'] %in% ctry_mapping$iso, ]
