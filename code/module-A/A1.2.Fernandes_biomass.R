...
# Reshape from wide to long format; drop rows with NA and 0 biomass
    ...
    names( raw )[ names( raw ) %in% c( "variable", "value" ) ] <- 
      c( "year", "consumption" )
...
# Drop all countries without ISO (i.e. those not in Master_Country_List)
    mapped$iso <- Master_Country_List$iso[ match( 
      mapped$country, Master_Country_List$Country_Name, nomatch = NA ) ]  # add ISO
    ...
    in_Fern_not_in_MCL <- filter( mapped, is.na( iso ) ) %>% unique() %>%
...
# b. If Fernandes falls below minimum value, bring up to minimum value
    ...
    full_min_fixed$consumption_pc[ full_min_fixed$consumption_pc < 
                                     full_min_fixed$consumption_pc_min ] <- 
      full_min_fixed$consumption_pc_min[ full_min_fixed$consumption_pc < 
                                           full_min_fixed$consumption_pc_min ]
