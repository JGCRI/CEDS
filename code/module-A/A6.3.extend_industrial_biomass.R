...
start_years <- c( IEA_start_year, 1971 )

iea_start_year$start_year <- 1971
iea_start_year[iea_start_year$iso %in% c( 'usa','aus','swe','can','prt','pol' ), 'start_year' ] <- IEA_start_year
...
ceds_biomass_full[paste0( 'X', IEA_start_year : end_year )] <- ceds_industrial_biomass_agg[match(ceds_biomass_full$iso, ceds_industrial_biomass_agg$iso ), paste0( 'X', IEA_start_year : end_year )]
...
breakdown_columns <- setdiff(breakdown_years, names(ceds_sector_breakdown))
ceds_sector_breakdown[breakdown_columns] <- NA_real_
...
# set up extension data (population)
  ...
  extension_data_1960[ paste0( 'X', historical_pre_extension_year : 1900 ) ] <- population[match( extension_data_1960$iso, population$iso ), paste0( 'X', 1750 : 1900 )]

  extension_data_1971 <- dissagregate_biomass_1971[,c('iso','sector','fuel')]
  extension_data_1971[ paste0( 'X', historical_pre_extension_year : 1900 ) ] <- population[match( extension_data_1971$iso, population$iso ), paste0( 'X', 1750 : 1900 )]
...
# Extend
  final_extended_1960 <- extend_data_on_trend(driver_trend = extension_data_1960, input_data = dissagregate_biomass_1960 , start = historical_pre_extension_year, end = 1849 )
  final_extended_1971 <- extend_data_on_trend(driver_trend = extension_data_1971, input_data = dissagregate_biomass_1971 , start = historical_pre_extension_year, end = 1849 )
  ...
  final_extended_biomass <- final_extended_biomass %>%
      dplyr::select( iso, sector, fuel, paste0( 'X', historical_pre_extension_year : 1970 ) ) %>%
...
# 10. Check final values
  ...
  if( anyNA( final_extended_biomass[paste0( 'X', historical_pre_extension_year : ( IEA_start_year - 1 ) ) ]) ){
