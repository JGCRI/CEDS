...
# Date Last Modified: 
# Program Purpose: Fix former soveit union energy data
# Details: first make hard coded changes to CEDS activity data
# Disaggregate total USSR data:
...
#----------------------------------- Hard Code Change ------------------------------------#
  ...
  corrected[which(corrected$fuel %in% 'coal_coke' ), paste0('X',1971:1977) ] <-   
                          corrected[which(corrected$fuel %in% 'coal_coke' ), 'X1978']
  ...
  # res = agciculture*(ag_redistribute)* res/(res+com+ind) + res
    corrected[ which( corrected$iso == FSU_iso[i] &
                        corrected$fuel == 'diesel_oil' &
                        corrected$sector == '1A4b_Residential') , paste0('X',1960:1989)] <- corrected[ which( corrected$iso == FSU_iso[i] &
                                                                                                          corrected$fuel == 'diesel_oil' &
  ...
  # industry-other = agciculture*(ag_redistribute)* ind/(res+com+ind) + industry-other
    corrected[ which( corrected$iso == FSU_iso[i] &
                        corrected$fuel == 'diesel_oil' &
                        corrected$sector == '1A2g_Ind-Comb-other') , paste0('X',1960:1989)] <- corrected[ which( corrected$iso == FSU_iso[i] &
                                                                                                                   corrected$fuel == 'diesel_oil' &
  ...
  # commericial = agciculture*(ag_redistribute)* com/(res+com+ind) + com
    corrected[ which( corrected$iso == FSU_iso[i] &
                        corrected$fuel == 'diesel_oil' &
                        corrected$sector == '1A4a_Commercial-institutional') , paste0('X',1960:1989)] <- corrected[ which( corrected$iso == FSU_iso[i] &
                                                                                                                   corrected$fuel == 'diesel_oil' &
...
# 4. Calculate USSR 1971-1990 share percentages and FSU population shares of total USSR
  ...
  USSR_percentages[X_IEA_years] <- replace( USSR_percentages[X_IEA_years] , is.na(USSR_percentages[X_IEA_years]) , 0 )


  # FSU population shares
...
# -------------------------------------------------------------------------------------------
  ...
  #  1971 - end_BP_year. Use BP data 1985 - 2013, then extend backward using population
  
  # Fuel shares for BP countries
  
  # percent of total USSR fuel (gas, coal, oil) each FSU country consumes
  ...
  FSU_countries_fuel_share_bp_extended[ paste0('X',1971:2013) ] <- t( na.locf( 
    t(FSU_countries_fuel_share_bp_extended[ paste0('X',1971:2013) ]), fromLast = TRUE )  )
  
  # Dissaggregate leftover Share to Non BP countries
  FSU_not_BP <- unique(FSU[which(FSU %!in% unique(bp_data$iso))])
  

  # population shares for each not BP country of the BP countries
  ...
  not_BP_pop_shares[,-1] <- not_BP_pop[,-1]/matrix( data = rep(colSums(not_BP_pop[,-1]), times = length(FSU_not_BP) ) , 
                                                    ...
                                                    byrow = T)  
  # use the population shares for each not BP country of the BP countries to dissagregate
  # left over
  #check FSU_leftover_shares not sure if its right
  not_BP_energy_shares <- list()
  for (i in seq_along(bp_fuel)){
    left_over_share <- as.numeric(1-colSums(FSU_countries_fuel_share_bp_extended[which(FSU_countries_fuel_share_bp_extended$bp_fuel == bp_fuel[i]),paste0('X',1971:end_BP_year)]) )
...
# write.csv(FSU_countries_fuel_share_ALL, 'FSU_countries_fuel_share_ALL.csv')
#   plot_df_data <- melt( FSU_countries_fuel_share_ALL )
#   plot_df_data$variable <- as.numeric(gsub('X','',plot_df_data$variable))
#   plot_df_data$bp_fuel <- as.factor(plot_df_data$bp_fuel)
#   for(i in seq_along( FSU_iso) ) {
#     iso_name <- FSU_iso[[i]]
#     plot_df <- plot_df_data[which(plot_df_data$iso == iso_name  ) , ]
#     
#     ggplot( data=plot_df , aes( x=variable, y=value, fill = bp_fuel))+
#       geom_area( data = plot_df, aes( x=variable, y=value, fill = bp_fuel))+
#       labs( x = 'Year', y = 'Fuel use [kt]' )+
#       ggtitle( paste( iso_name) )+
#       scale_x_continuous(breaks=seq(from=1970,to=2014,by=5))
#     
#     ggsave( paste0( '../diagnostic-output/FSU-diagnostics/FSU_fuel_share_', iso_name ,'.pdf') ,
#             width = 7.5, height = 4)
#   }
  
  # gives df of shares of total fuel use (oil, coal gas) for each FSU country with USSR activtiy 
  # data to give total fuel use (oil coal gas) for each FSU country
  
# -------------------------------------------------------------------------------------------
# 7. FSU countries, calculate sector share of each ceds fuel
  
  # Calculate USSR sector share of each fuel
  ...
  for (i in seq_along(agg_fuel)){
    ...
    USSR_sector_by_fuel_df[,paste0('X',1971:2013)] <- USSR_sector_by_fuel_df[,paste0('X',1971:2013)] / matrix( data = rep( colSums(USSR_sector_by_fuel_df[,paste0('X',1971:2013)]), times = nrow(USSR_sector_by_fuel_df)) , 
  ...
  # Calculate FSU sector share of each fuel
  ...
  FSU_iso_agg_sector_agg_fuel <- aggregate( activity[ paste0('X',1990:2013) ],
                                            ...
                                            FUN = sum )
  # create data frame with all iso-agg_sector-agg_fuels and fill with zeros, then replace values
...
#   #plot
#   plot_data <- melt(FSU_iso_agg_sector_agg_fuel)
#   plot_data$variable <- as.numeric(gsub('X','',plot_data$variable))
#   for (i in seq_along(agg_fuel)){
#     for (n in seq_along(FSU_iso)){
#       data_df <- plot_data[ which( plot_data$iso == FSU_iso[n] & plot_data$fuel == agg_fuel[i] ) , ] 
#       # pdf(paste0( '../diagnostic-output/FSU-diagnostics/FSU-sector-shares-by-fuel/sector_shares_by_fuel_', FSU_iso[n] ,'_', agg_fuel[i] ,'.pdf'))
#       qplot( data = data_df, x=variable, y=value, geom = 'line', color = sector,
#              xlab = 'Year', ylab = 'Fuel [kt]', main = paste('Sector share by fuel -',FSU_iso[n],':',agg_fuel[i]))+
#         scale_x_continuous(breaks=seq(from=1990,to=2014,by=2))
#       # dev.off()
#       ggsave( paste0( '../diagnostic-output/FSU-diagnostics/FSU-sector-fuel-use-by-fuel/sector_fuel_use_by_fuel_', FSU_iso[n] ,'_', agg_fuel[i] ,'.pdf') ,
#               width = 7.5, height = 4)
#     }}
  ...
  USSR_sector_shares_by_fuel[ which( USSR_sector_shares_by_fuel$agg_fuel == 'heavy_oil' &
                                       USSR_sector_shares_by_fuel$agg_sector %in% c('Power' , 'Industry' ) ),paste0('X',1973:1988)  ] <- NA
  # diesel_oil
  USSR_sector_shares_by_fuel[ which( USSR_sector_shares_by_fuel$agg_fuel == 'diesel_oil' &
                                       USSR_sector_shares_by_fuel$agg_sector %in% c('Agriculture' , 'Industry' ) ), paste0('X',1975:1989)  ] <- NA
  ...
  for (i in seq_along(FSU_iso)){
    ...
    shares[paste0('X',1971:2013)] <- shares[paste0('X',1971:2013)]/matrix( 
                            data = rep( sum , times =  nrow(shares) ),
...
#   write.csv(FSU_iso_sector_shares_by_fuel_extended, 'FSU_iso_sector_shares_by_fuel_extended.csv')  
# 
#     #plot
#   plot_df_data <- melt(FSU_iso_sector_shares_by_fuel_extended, id.vars = c('iso','fuel','sector'))
#   plot_df_data$variable <- as.numeric(gsub('X','',plot_df_data$variable))
#   for(i in seq_along( FSU_iso) ) {
#     plot_list <- list()
#     for (n in seq_along(agg_fuel) ){
...
#       plot_df <- plot_df_data[which(plot_df_data$iso == iso_name & plot_df_data$fuel == fuel_name ) , ]
#       
#       plot_list[[n]] <- ggplot( data=plot_df , aes( x=variable, y=value, fill = sector ))+
#         geom_area( )+
#         labs( x = 'Year', y = 'Percent' )+
#         ggtitle( paste( fuel_name) )+
#         scale_x_continuous(breaks=seq(from=1970,to=2014,by=5))
#     }
#     pdf(paste0( '../diagnostic-output/FSU-diagnostics/FSU-sector-shares-by-fuel/FSU_sector_share_',iso_name,'.pdf'),
#         width=12,height=10,paper='special')
#     grid.arrange(plot_list[[1]],plot_list[[2]],
#                  plot_list[[3]],plot_list[[4]],
#                  plot_list[[5]],plot_list[[6]], ncol=2,
#                  top = paste('New Activity Data:',iso_name))
#     dev.off()
#   }
    
# 
  # -------------------------------------------------------------------------------------------
  # 9. FSU countries, aggregate activity data (no sector breakdown) . first activity by bp fuel then ceds aggregate fuel
  #FSU_countries_fuel_share_ALL
...
#   plot_df_data <- melt( FSU_fuel_use_by_BPfuel )
#   plot_df_data$variable <- as.numeric(gsub('X','',plot_df_data$variable))
#   plot_df_data$bp_fuel <- as.factor(plot_df_data$bp_fuel)
#   for(i in seq_along( FSU_iso) ) {
#     iso_name <- FSU_iso[[i]]
#     plot_df <- plot_df_data[which(plot_df_data$iso == iso_name  ) , ]
#     
#     ggplot( data=plot_df , aes( x=variable, y=value, color = bp_fuel))+
#       geom_line( data = plot_df, aes( x=variable, y=value, color = bp_fuel))+
#       labs( x = 'Year', y = 'Fuel use [kt]' )+
#       ggtitle( paste( iso_name) )+
#       scale_x_continuous(breaks=seq(from=1970,to=2014,by=5))
#     
#     ggsave( paste0( '../diagnostic-output/FSU-diagnostics/FSU-dissaggregated-bp-fuel/FSU_dissaggregated_bp_fuel_', iso_name ,'.pdf') ,
#             width = 7.5, height = 4)
#   }
#   
  ...
  FSU_fuel_agg_fuel_fromUSSRbp <- do.call(rbind, iso_list) # dissagregated to FSU. fuel totals by agg fuel, no sector
  
#   # write.csv(FSU_fuel_agg_fuel_fromUSSRbp, 'FSU_fuel_agg_fuel_fromUSSRbp.csv')
#   # plot
#   ##### plot new data
#   plot_df_data <- melt(FSU_fuel_agg_fuel_fromUSSRbp[,c('iso','agg_fuel',paste0('X',1971:2013))] )
#   plot_df_data$variable <- as.numeric(gsub('X','',plot_df_data$variable))
#   for(i in seq_along( FSU_iso) ) {
#       iso_name <- FSU_iso[[i]]
#       plot_df <- plot_df_data[which(plot_df_data$iso == iso_name ) , ]
#      
#     ggplot( data=plot_df , aes( x=variable, y=value, color = agg_fuel ))+
#         geom_line( )+
#         labs( x = 'Year', y = 'Fuel use [kt]' )+
#         ggtitle( paste( iso_name) )+
#         scale_x_continuous(breaks=seq(from=1970,to=2014,by=5))
# 
#     ggsave( paste0( '../diagnostic-output/FSU-diagnostics/Dissaggregated_Data_aggfuel_',iso_name,'.pdf') ,
#                width = 7.5, height = 4)
#   }
  ...
  # -------------------------------------------------------------------------------------------
  # 6. Disaggregate FSU new activity from fuel totals to each sector by Sector Share
...
#   # write.csv(FSU_disaggregated_activity_agg, 'FSU_disaggregated_activity_agg.csv')
#   # plot
#   plot_df_data <- melt(   FSU_disaggregated_activity_agg , id.vars = c('iso','agg_fuel','agg_sector'))
#   plot_df_data$variable <- as.numeric(gsub('X','',plot_df_data$variable))
#   for(i in seq_along( FSU_iso) ) {
#     plot_list <- list()
#     for (n in seq_along(agg_fuel) ){
#       iso_name <- FSU_iso[[i]]
#       fuel_name <- agg_fuel[[n]]
#       plot_df <- plot_df_data[which(plot_df_data$iso == iso_name & plot_df_data$agg_fuel == fuel_name ) , ]
#       
#       plot_list[[n]] <- ggplot( data=plot_df , aes( x=variable, y=value, color = agg_sector ))+
#         geom_line( )+
#         labs( x = 'Year', y = 'Percent' )+
#         ggtitle( paste( fuel_name) )+
#         scale_x_continuous(breaks=seq(from=1970,to=2014,by=5))
#     }
#     pdf(paste0( '../diagnostic-output/FSU-diagnostics/FSU-dissaggregated-agg-fuel/Dissaggregated_aggfuel_',iso_name,'.pdf'),
#         width=12,height=10,paper='special')
#     grid.arrange(plot_list[[1]],plot_list[[2]],
#                  plot_list[[3]],plot_list[[4]],
#                  plot_list[[5]],plot_list[[6]], ncol=2,
...
#   
  # -------------------------------------------------------------------------------------------
# 6. Blend Diaggregated USSR data and reported CEDS data
...
#   # ------------------------------------------------------------------#
#   # write.csv(FSU_activity_agg_corrected, 'FSU_activity_agg_corrected.csv')
#   ##### plot new data
#   plot_df_data <- melt(FSU_activity_agg_corrected, id.vars = c('iso','agg_fuel','agg_sector'))
#   plot_df_data$variable <- as.numeric(gsub('X','',plot_df_data$variable))
#   for(i in seq_along( FSU_iso) ) {
#     plot_list <- list()
#     for (n in seq_along(agg_fuel) ){
...
#         scale_x_continuous(breaks=seq(from=1970,to=2014,by=5))
#     }
#     pdf(paste0( '../diagnostic-output/FSU-diagnostics/New_Activity_Data_',iso_name,'.pdf'),
#         width=12,height=10,paper='special')
#     grid.arrange(plot_list[[1]],plot_list[[2]],
#                  plot_list[[3]],plot_list[[4]],
#                  plot_list[[5]],plot_list[[6]], ncol=2,
...
#    
# -------------------------------------------------------------------------------------------
# X. Dissaggregate corrected Data 
...
# Disaggregate from agg_sectors to sectors
