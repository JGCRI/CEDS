...
# define function to correct coal shares over IEA data
  fix_hard_coal_shares <- function (df) {
    ...
    if ( df[ which( df$PRODUCT == 'Hard coal' ),  'X1977' ] != 0 &
      ...
      # combine all rows
      corrected_all <- rbind( out, extended, hard_coal) %>% arrange( PRODUCT)
      #renormalize to one
...
# Conversion years
  conversion_years <-  select(conversion_all, contains ('X') ) %>% names
...
# Calculate shares of energy data for countries by fuel type
  ...
  coal_shares <- calculate_shares (input_data = coal_data, 
                                   id_columns= 'iso_fuel',
                                   target_column = 'PRODUCT',
                                   replace_with_zeros = T) %>% separate("iso_fuel", c("iso","fuel"), sep = "-")
...
# Map heat content to iso
  ...
  conversion_extended_iso <- conversion_extended_iso %>% drop_na(iso, fuel, PRODUCT) 
...
hc_coal <- weighted_average_heat_content %>% mutate(units = 'kJ/kg')
