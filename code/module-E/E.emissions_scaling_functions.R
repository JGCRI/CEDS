...
# Read in data    
    ...
    ef_file <- paste0( "C.", em_species, "_total_scaled_EF" )
    em_file <- paste0( "C.", em_species, "_total_scaled_emissions" )
...
# Clean the data
    clean_input_ef <- removeBlanks( input_ef, 'iso' )
    clean_input_em <- removeBlanks( input_em, 'iso' )
...
# Apply ceds scaling to the most recent scaled emissions factors  
    ...
    scaled_ef <- removeBlanks( input_ef, 'iso' )
...
# Apply ceds scaling to the most recent scaled emissions
    scaled_region_em <- mapply( '*', region_input_em,
        ceds_scaling[ , 2:ncol( ceds_scaling ) ] )
    scaled_em <- removeBlanks( input_em, 'iso' )
