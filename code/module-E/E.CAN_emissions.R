...
#                  favor of the newer data. This uses the CAN_2018 scaling mapping
# Input Files: EN_APEI_Can_Prov_Terr.csv
# Output Files: E.[em]_CAN_2021_inventory.csv, E.[em]_CAN_inventory_country_total.csv
# Notes:
...
# provide logging, file support, and system functions - and start the script log.
  ...
  inv_years <- c( 1990:2022 )
  # Because this data comes read in as reversed.
  inv_years_reversed <- c( 2022:1990 )
  # raw data file name
  raw_folder_name <- 'raw_2022'
  ...
  output_data <- function(em2) {
      ...
      raw_data_path <- paste0('emissions-inventories/Canada/',raw_folder_name,'/',em2,'/')
  ...
  if( em %!in% c("CO2", "CH4", "N2O") ){
      ...
      inv_iso <- "can"
      PM <- "PM25"
      mapping_file <- readData("SCALE_MAPPINGS", "CAN_scaling_mapping.csv")
