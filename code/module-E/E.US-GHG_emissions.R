...
# select rows and columns 
  inv_data <- inv_data_sheet[3:nrow(inv_data_sheet),2:ncol(inv_data_sheet)]
  names(inv_data) <- inv_data_sheet[2,2:ncol(inv_data_sheet)]
  names(inv_data)[1] <- 'Source'
  inv_data <- inv_data[-grep("Does not exceed",inv_data[,1]):-nrow(inv_data_sheet),]

# Define emission species
  inv_data$EM <- NA
  inv_data[inv_data$Source %in% ghg_em,'EM'] <- inv_data[inv_data$Source %in% ghg_em,'Source']
...
# convert to numeric
  inv_data[paste0('X', inv_years)] <- apply(X=inv_data[paste0('X', inv_years)],2,FUN = gsub,pattern = ',',replacement='')
